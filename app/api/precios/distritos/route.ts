...
export async function POST(request: NextRequest) {
  ...
  try {
    const { codigoDepartamento, codigoProvincia } = distritosSchema.parse(raw);
    const key = cacheKey("snippf_dist", [
      codigoDepartamento,
      codigoProvincia,
    ]);
    ...
    const data = await cacheRemember(key, CACHE_TTL.ubigeo, async () => {
      try {
        ...
        if (err instanceof DigemidHttpError) {
          console.warn("[precios/distritos]", err.message);
        }
        return [];
      }
    });
    ...
    console.error("[precios/distritos]", err);
    return jsonFail("Error inesperado al listar distritos.", {
      status: 500,
      userId: rl.userId,
      isNew: rl.isNew,
    });
